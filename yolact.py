...
class FPN(ScriptModuleWrapper):
    ...
    def __init__(self, in_channels):
        ...
        print('self.lat_layers:', self.lat_layers)
    ...
    def __init__(self):
        ...
        if cfg.mask_type == mask_type.direct:
            ...
            if cfg.mask_proto_bias:
                cfg.mask_dim += 1
           # print('self.proto_net:', self.proto_net)
           # print('cfg.mask_dim:', cfg.mask_dim)

        ###在这里开始cfg.backbone.selected_layers， 这里我需要对齐进行改进
        self.selected_layers = cfg.backbone.selected_layers
        #print('self.selected_layers:', self.selected_layers)
        src_channels = self.backbone.channels
       # print('src_channels:', src_channels)
       # print([src_channels[i] for i in self.selected_layers])
        if cfg.use_maskiou:
            self.maskiou_net = FastMaskIoUNet()
        if cfg.fpn is not None:
            if cfg.name == 'yolact_hrnet18':
                selected_layer = self.selected_layers[0]
                self.fpn = HRFPN(src_channels[selected_layer])
                src_channels = [cfg.fpn.num_features] * 5
        ...
        for idx, layer_idx in enumerate(self.selected_layers):
           ...
           # print('aspect_ratios:',cfg.backbone.pred_aspect_ratios[idx])
           # print('scales:', cfg.backbone.pred_scales[idx])
            #print('src_channels:', src_channels[layer_idx])
...
#                     del state_dict[key]
    ...
    def forward(self, x):
        ...
        with timer.env('backbone'):
            ...
            #print('finished forward training, the len(outs):',len(outs))
#             print('new outs[2]:', len(outs[2]))

        #####在forward这里要做一下文章
        if cfg.fpn is not None:
            with timer.env('fpn'):
                # Use backbone.selected_layers because we overwrote self.selected_layers
                #print('backbone.selected_layers:', cfg.backbone.selected_layers)
                outs = [outs[i] for i in cfg.backbone.selected_layers]
                #print('before fpn the len(outs):', len(outs[0]))
                outs = self.fpn(outs)
               # print('after fpn the len(outs):', len(outs))

        ######outs为backbone的输出，还需要先知道backbone是个啥
