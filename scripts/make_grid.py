...
def render():
    ...
    x = (
        np.tile(
            np.array(list(range(grid_w)), dtype=np.float).reshape(1, grid_w),
            [grid_h, 1],
        )
        - grid_w * center_x
    )
    y = (
        np.tile(
            np.array(list(range(grid_h)), dtype=np.float).reshape(grid_h, 1),
            [1, grid_w],
        )
        - grid_h * center_y
    )
    ...
    for y in range(grid_h):
        for x in range(grid_h):
            ...
            l1_dist = np.sum(
                np.abs(
                    grids
                    - np.tile(pixel_features, grid_h * grid_w).reshape(
                        (-1, grid_h, grid_w)
                    )
                ),
                axis=0,
            ...
            if num_equal > 1:
                print(
                    "Pixel at (%d, %d) has %d other pixel%s with the same representation."
